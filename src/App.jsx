    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(30,27,75,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 16 }} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={{ background: "white", borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 420, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(99,102,241,0.25)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
          <h2 style={{ margin: 0, fontFamily: "'Playfair Display', serif", color: "#1e1b4b", fontSize: "1.3rem" }}>Wybierz plan</h2>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
          {PLANS.map(plan => (
            <div key={plan.id} onClick={() => setSelectedPlan(plan.id)} style={{ border: selectedPlan === plan.id ? "2px solid #4f46e5" : "2px solid #e0e7ff", borderRadius: 12, padding: "12px 16px", cursor: "pointer", background: selectedPlan === plan.id ? "#f5f3ff" : "white", transition: "all 0.15s" }}>
        ...
        {showTokenField && (
          <div style={{ borderTop: "1px solid #e0e7ff", paddingTop: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button onClick={() => token.trim() && onEnterToken(token.trim())} style={{ background: "#4f46e5", color: "white", border: "none", borderRadius: 10, padding: "8px 16px", cursor: "pointer", fontSize: "0.88rem", fontWeight: 600 }}>Wejdź</button>
            </div>
          </div>
        )}
      </div>
...
export default function KSeFAsystent() {
  ...
  const handleImageUpload = (e) => {
    ...
    if (!file) return;
    if (!userToken) { setMessages(prev => [...prev, { role: "assistant", content: "Analiza faktur dostępna jest w płatnych planach." }]); if (fileInputRef.current) fileInputRef.current.value = ""; return; }
    if (!["image/jpeg","image/png","image/webp","image/gif","application/pdf"].includes(file.type)) { setMessages(prev => [...prev, { role: "assistant", content: "Obsługiwane formaty: JPG, PNG, WebP, PDF." }]); if (fileInputRef.current) fileInputRef.current.value = ""; return; }
    if (file.size > 5*1024*1024) { setMessages(prev => [...prev, { role: "assistant", content: "Plik jest za duży. Maksymalny rozmiar to 5 MB." }]); if (fileInputRef.current) fileInputRef.current.value = ""; return; }
    const reader = new FileReader();
    reader.onload = (ev) => { const base64 = ev.target.result.split(",")[1]; setImage({ base64, mediaType: file.type, isPdf: file.type === "application/pdf", fileName: file.name }); setImagePreview(file.type === "application/pdf" ? null : ev.target.result); };
    reader.readAsDataURL(file);
  };

  const removeImage = () => { setImage(null); setImagePreview(null); if (fileInputRef.current) fileInputRef.current.value = ""; };
  ...
  const sendMessage = async (text) => {
    ...
    let userMessage;
    if (image) {
      const cb = image.isPdf ? { type: "document", source: { type: "base64", media_type: "application/pdf", data: image.base64 } } : { type: "image", source: { type: "base64", media_type: image.mediaType, data: image.base64 } };
      userMessage = { role: "user", content: [cb, { type: "text", text: userText || "Sprawdź tę fakturę." }] };
    } else { userMessage = { role: "user", content: userText }; }
    const newMessages = [...messages, userMessage];
    setMessages(prev => [...prev, { role: "user", content: userText || "Sprawdź tę fakturę.", hasImage: !!image, imagePreview }]);
    removeImage();
    setLoading(true);
    try {
      const response = await fetch("/api/chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: newMessages.map(m => ({ role: m.role, content: m.content })), userToken: userToken || null, messageCount, hasImage: !!image }) });
    ...
    } catch { setMessages(prev => [...prev, { role: "assistant", content: "Problem z połączeniem. Sprawdź internet i spróbuj ponownie." }]); }
    finally { setLoading(false); inputRef.current?.focus(); }
  };

  const handleKey = (e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(); } };
  const handleEnterToken = (token) => { setUserToken(token); setShowPaywall(false); setShowPricing(false); setMessages(prev => [...prev, { role: "assistant", content: "Kod przyjęty — masz pełny dostęp. Czym mogę pomóc?" }]); };
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #eef2ff 0%, #f5f3ff 50%, #ede9fe 100%)", fontFamily: "'Source Serif 4', Georgia, serif", display: "flex", flexDirection: "column", alignItems: "center", paddingBottom: 32 }}>
      <style>{`
        ...
        * { box-sizing: border-box; } textarea:focus, input:focus { outline: none; }
        ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-thumb { background: #c7d2fe; border-radius: 10px; }
        .quick-btn:hover { background: #4f46e5 !important; color: white !important; } .send-btn:hover { background: #4338ca !important; }
      ...
      <div style={{ width: "100%", background: "linear-gradient(135deg, #3730a3, #4f46e5, #6366f1)", padding: "20px 20px 16px", textAlign: "center", position: "sticky", top: 0, zIndex: 10, boxShadow: "0 4px 20px rgba(79,70,229,0.25)" }}>
        ...
        {!isPaid && <div style={{ marginTop: 10, background: "rgba(255,255,255,0.15)", borderRadius: 20, padding: "4px 14px", display: "inline-block", fontSize: "0.78rem", color: "white" }}>{remainingFree > 0 ? `Pozostało ${remainingFree} z ${FREE_LIMIT} bezpłatnych wiadomości` : "Limit bezpłatnych wiadomości wyczerpany"}</div>}
        {isPaid && <div style={{ marginTop: 10, background: "rgba(255,255,255,0.15)", borderRadius: 20, padding: "4px 14px", display: "inline-block", fontSize: "0.78rem", color: "#a5f3fc" }}>Pełny dostęp</div>}
      </div>
      ...
      {messages.length <= 1 && !showPaywall && (
        <div style={{ maxWidth: 680, width: "calc(100% - 32px)", margin: "20px 16px 0", animation: "slideIn 0.6s ease" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {QUICK_QUESTIONS.map((q, i) => <button key={i} className="quick-btn" onClick={() => sendMessage(q.text)} style={{ background: "white", border: "1.5px solid #c7d2fe", borderRadius: 20, padding: "8px 14px", fontSize: "0.82rem", color: "#3730a3", cursor: "pointer", transition: "all 0.15s", fontFamily: "inherit" }}>{q.text}</button>)}
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth: 680, width: "calc(100% - 32px)", margin: "20px 16px 0", flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        {loading && (
          <div style={{ display: "flex", justifyContent: "flex-start", animation: "fadeIn 0.3s ease" }}>
            ...
            <div style={{ background: "white", borderRadius: "18px 18px 18px 4px", padding: "12px 16px", boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}><TypingIndicator /></div>
          </div>
        )}
